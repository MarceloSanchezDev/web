...
function App() {
  ...
  return <><Shell user={session.user} page={page} setPage={key => navigate(pagePath(key))} logout={handleSignOut}>
    ...
    {page === 'equipos' && <TeamWorkspace api={api} director={session.user.role === 'DIRECTOR'} readOnly={session.user.role === 'MONITOR'} notify={notify} teamId={route.id} tab={route.tab} navigate={navigate} />}
